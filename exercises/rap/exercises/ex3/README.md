...
## Exercise 3.1: Provide Metadata Extension for the SAP Fiori elements UI
 
1. In the Project Explorer, under the **Metadata Extensions** section, double-click on the **ZC_ONLINESHOP_###** file.   

![enhance_data_model](images/115_adapt_data_model.png) 

2. Replace the complete source code of the file with the code below. Remember to replace the placeholder `###` with the 3-digit code in your ABAP username.    
...
We have to create determinations for fields that are read-only and that are not read from the value help. 
  
  1. Open the behavior definition `ZR_ONLINESHOP_###`.  

  ![adapt_bdef](images/195_adapt_bdef.png)  

  2. In the `field ( readonly)` section, add the following list of fields to mark them as read-only:  
  <pre lang="ABAP">
   OrderID,
   PurchaseRequisition,
   OverallStatus,
  ...
  3. After the `field ( numbering : managed )` section, add the following additional section:
      
  <pre lang="ABAP">
        field ( mandatory:create, readonly:update ) Product;
   </pre>

   This will make the `Product` mandatory and editable on `Create` and read-only during `Edit` operations.
  
  ![adapt_bdef](images/220_adapt_bdef.png)  
 
  3. Save ![save icon](../../images/adt_save.png) and activate ![activate icon](../../images/adt_activate.png) the changes.

  4. In the **Project Explorer**, go to `Business Services`->`Service Bindings`->`ZUI_ONLINESHOP_O4_###`.
     
  5. Double-click `ZUI_ONLINESHOP_O4_###` and in the view that opens, click **Preview...** to see the UI using the SAP Fiori elements preview. 

  6. In the preview tab, press **Create** on the right of the page. You should see a form like this, where **Order ID**, **Purchase Requisition** and **Overall Status** are text not input fields:
...
## Exercise 3.3: Define Determinations
  ...
  We now will create a determination which is called when a new online store entry is created. The determination will calculate a new Order ID. While the OrderUUID is generated automatically by the system upon save, we have to generate the Order ID ourselves. The corresponding code looks up the currently highest number for orders, and then adds 1 for a new ID.
 
  1. In the **Project Explorer**, go to **Core Data Services** -> **Behavior Definitions** -> **ZR_ONLINESHOP_###**.
  2. Add the following determination to your **ZR_ONLINESHOP_###** behavior definition ()
  ...
  3. Press **Ctrl+1** (**Command+1** on Mac) while the cursor is on “CalculateOrderID” to open the quick fix, and double-click on “Add method for determination calculateorderid of entity…” to generate the appropriate methods in the behavior definition class.

  ![define_determinations](images/310_define_determinations.png) 

  A new tab opens with the generated handler that looks like this:
...
"ignore  entries with assigned order ID
  ...
  The code reads all entries including the one that was just created. It looks at all the orders and checks if they already have an `OrderID`, and it finds the order with the highest `OrderID`, assuming this is the newest. It adds 1 to the highest `OrderID`, assigns this new number to our new onlineshop record, and modifies the database using EML ([Entity Manipulation Language](https://help.sap.com/docs/btp/sap-abap-restful-application-programming-model/entity-manipulation-language-eml))
...
1. In the **Project Explorer**, go to **Source Code Library** -> **Classes** -> **`ZBP_R_ONLINESHOP_###`**.
2. Add the following line to the `ZBP_R_ONLINESHOP_###` file after the `public section.` line. (Make sure to switch to the `Global Class` tab at the bottom of the ADT editor)
...
3. In the **Project Explorer**, go to **Core Data Services** -> **Behavior Definitions** -> **ZR_ONLINESHOP_###**.
4. Add the following internal action just behind the line for the calculation of the new order ID, that you inserted before.
...
8. Add the following code to the end of the **CalculateOrderID** method, before the **ENDMETHOD** statement.
   <pre lang="ABAP">
...
This will generate the "Purchase Requisition Created" standard event. You will consume this event later in SAP Build Code for further processing. **[Build Exercise 4: SAP Add and consume an event from S/4HANA On-Premise](../../../buildcode/exercises/ex4/README.md)**

## Exercise 3.5: Add unmanaged save

Since the Purchase Requisition API that we plan to call uses late numbering, we have to add the use of an unmanaged save to the behavior of our RAP business object.

1. In the **Project Explorer**, go to **Core Data Services** -> **Behavior Definitions** -> **ZR_ONLINESHOP_###** (or open it as a development object using the short cut Ctrl+Shift+A).

2. Open the behavior definition, and add the following statement: 
...
3. In addition, comment out the statement that specifies the persistent table of our RAP BO `//persistent table zaonlineshop_###` since it is not possible to use both statements in the same behavior definition. 
...
## Exercise 3.6: Check your preview application
 ...
 1. In the **Project Explorer**, go to **Business Services** -> **Service Bindings** -> **`ZUI_ONLINESHOP_O4_###`** 
 2. Open the `ZUI_ONLINESHOP_O4_###` service binding, and click **Preview** in the ADT editor to test your implementation. Alternatively, if you kept the browser window open with the SAP Fiori elements preview, you can just refresh the browser and it will automatically reflect the new code.

 3. On the list, press `Create`, and then. on the object page, enter a new onlineshop entry.
    a. In the **Create** dialog, enter `D001` for the Product, and then press **Continue**.
    b. In the following object page, enter `3` for the Quantity, and then press **Create**. in lower-right corner.
 ![define_determinations](images/313_0_define_determinations.png) 
 ![define_determinations](images/313_define_determinations.png) 
  
  
  The order ID is calculated, and a new purchase requisition is created in SAP S/4HANA. The result should look like this:
 ...
 You have implemented an adjustment to the SAP Fiori elements UI and a determination to calculate a new order ID and to create a purchase requisiton in SAP S/4HANA.   
